...
export default function Reports() {
  ...
  useEffect(() => {
    api.get("/scans")
      .then((response) => setScans(asArray(response.data)))
  ...
  const summarize = async (scan) => {
    ...
    try {
      ...
      setSummary(response.data?.summary || "No summary was returned.");
    } catch (err) {
      setSummary(getApiError(err, "Executive summary failed"));
    }
  };
            ...
            <div key={scan.id} className="glass rounded-lg p-5">
              <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">{asArray(scan.findings).length} findings | posture score {scan.risk_score}</p>
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => downloadReport(scan).catch((err) => setError(getApiError(err, "Report download failed")))} className="inline-flex items-center gap-2 rounded-lg border border-cyan/30 px-3 py-2 text-sm font-semibold text-cyan hover:bg-cyan/10">
